...
// (function ($, window, document) {
    ...
    $.extend(Plugin.prototype, {
        init() {
            ...
            // Bind events
            _this.$t.children("thead").first().children("tr").find(".shrink-toggle-all").on("click", _this.toggleAll.bind(_this)); // toggle-all for main table

            _this.$t.find(".shrinked-row.shrink-has-table table>thead>tr>.shrink-toggle").on("click", _this.toggleAll.bind(_this)); // toggle-all for shrinked tables

            if (_this.$t.parents("table").length === 0) { _this.$t.children("tbody").first().children("tr").find(".shrink-toggle").on("click", _this.toggle.bind(_this)); }

            if (_loadCollapsed) { _this.$t.find(">thead>tr>.shrink-toggle").click(); }
        },
        toggle(e) {
